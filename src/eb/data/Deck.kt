...
class Deck(val name: String) : Serializable {
    ...
    private fun createTextFile(fileName: String, formatter: (Card) -> String) {
        try {
            ...
            // write the header
            bw.write("Eb version ${Eb.VERSION_STRING}$EOL")
            bw.write("Number of cards is: ${cardCollection.getTotal()}$EOL")
            bw.write(HEADER_BODY_SEPARATOR + EOL)

            // write the card data
            cardCollection.writeCards(bw, formatter)
        } catch (e: Exception) {
            log(e.toString())
    ...
    private fun getIntervalAfterSuccessfulReview(card: Card): Duration {
        ...
        return Utilities.multiplyDurationBy(waitTime, pow(lengtheningFactor, numberOfLengthenings.toDouble()))
    }
