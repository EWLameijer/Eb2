...
object CardConverter {
    ...
    private fun lineToCard(line: String): Card {
        val strings = line.split(SEPARATOR_REGEX).dropLastWhile { it.isEmpty() }.toTypedArray()
    ...
    /**
     * The cardToLine method transforms the data of a card to a line, for purposes of saving it to a human-readable file
     * (that is also easy to restore data from).
     *
     * @param card the card to be written into a line of text.
     * @return a newline-terminated String containing the front and back of the card, as text.
    */
     ...
     * complexity)
     *
     * @param card  the card of which the review history must be saved.
    ...
    */
    ...
    fun extractCardsFromArchiveFile(selectedFile: File) {
        try {
            val lines =
                    Files.readAllLines(selectedFile.toPath(), Charset.forName("UTF-8"))

            val cardCollection = DeckManager.currentDeck().cardCollection
            lines.takeLastWhile { it != SEPARATOR }.forEach { cardCollection.addCard(lineToCard(it)) }
        } catch (e: IOException) {
